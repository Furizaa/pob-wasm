...
fn add_item_mods(build: &Build, env: &mut CalcEnv) {
    ...
    for (slot_name, &item_id) in &item_set.slots {
        ...
        //
        // Post-processing: mods from "socketed <keyword> gems" prefixes (e.g.
        // "Socketed Curse Gems have 12% reduced Mana Reservation") are generated by
        // the Lua parser with `addToSkill = { type = "SocketedIn", slotName = slot,
        // keyword = "<kw>" }`.  The Rust generated parser emits these with no tags and
        // no keyword_flags, which causes them to apply globally.
        //
        // We fix this by detecting the prefix on each raw line and adding:
        //   1. keyword_flags = the appropriate KeywordFlags constant
        //   2. a SocketedIn { slot_name } tag
        // so that the mod is filtered correctly when the player mod_db is queried
        // with per-skill SkillCfg (which carries keyword_flags and slot_name).
        let all_mods: Vec<Mod> = mod_lines
            .flat_map(|line| {
                let mods = crate::build::mod_parser::parse_mod(line, source.clone());
                let line_lower = line.to_lowercase();
                // Detect "socketed <keyword> gems have/gain/deal ..." prefix patterns.
                // Mirrors ModParser.lua lines 1193–1204.
                let socketed_kw = if line_lower.starts_with("socketed curse gems ") {
                    Some(crate::mod_db::types::KeywordFlags::CURSE)
                } else if line_lower.starts_with("socketed spell gems ")
                    || line_lower.starts_with("socketed spells ")
                {
                    Some(crate::mod_db::types::KeywordFlags::SPELL)
                } else if line_lower.starts_with("socketed attack ") {
                    Some(crate::mod_db::types::KeywordFlags::ATTACK)
                } else if line_lower.starts_with("socketed gems ") {
                    // Generic "socketed gems" — no keyword restriction, but still needs SocketedIn
                    Some(crate::mod_db::types::KeywordFlags::NONE)
                } else {
                    None
                };
                if let Some(kw) = socketed_kw {
                    mods.into_iter()
                        .map(|mut m| {
                            // Only augment mods that came from addToSkill patterns.
                            // These typically have no tags and no keyword_flags, and
                            // have a non-List mod type (INC/Base/More/etc.).
                            if m.mod_type != crate::mod_db::types::ModType::List
                                && !m
                                    .tags
                                    .iter()
        ...
        // Build src_list: non-local-defence mods, with SocketedIn tags injected on
        // ExtraSupport mods that lack one.
        //
        // In Lua (ModParser.lua:2010), all ExtraSupport mods generated from item affixes
        // carry a `{ type = "SocketedIn", slotName = slot }` tag so that they only apply
        // to skill groups socketed in that specific slot (e.g. "Helmet").
        //
        // The Rust mod_parser_generated.rs and mod_parser_manual.rs currently emit
        // ExtraSupport mods with `tags: vec![]` (no SocketedIn tag).  We fix that here
        // by adding the SocketedIn tag for any ExtraSupport mod that doesn't already
        // have one, using the current `slot_name` as the slot.
        //
        // This ensures that when Phase 4 of active_skill.rs queries
        //   mod_db.list("ExtraSupport", &cfg_with_slot, &output)
        // with cfg.slot_name = "Helmet", only ExtraSupport mods from items in the Helmet
        // slot are returned (Blasphemy for Heretic's Veil, etc.).
        let src_list: Vec<Mod> = all_mods
            .iter()
            .filter(|m| !is_local_defence_mod(m))
            .cloned()
            .map(|mut m| {
                if m.name == "ExtraSupport"
                    && !m
                        .tags
                        .iter()
                        .any(|t| matches!(t, ModTag::SocketedIn { .. }))
