...
```
You are rewriting the PERF-04 reservation accumulation to use the
fully-constructed active_skill_list. Work on chunk PERF-04-reservation.

THE LUA SOURCE IS THE SOURCE OF TRUTH. The previous implementation of
accumulate_skill_reservations iterated raw XML gem data instead of the
active_skill_list. This MUST be rewritten.

Context:
1. Read docs/lua-reference/PERF-04-reservation.md
2. Read CalcPerform.lua — find the reservation section (lines ~1810-1919).
   Note how the Lua iterates env.player.activeSkillList and reads:
   - skill.skillFlags.aura / skill.activeEffect.grantedEffect.support
   - skill.skillData.manaReservationPercent / manaReservationFlat
   - skill.skillData.lifeReservationPercent / lifeReservationFlat
   - skill.buffModList for reservation efficiency mods
3. Read the current accumulate_skill_reservations in perform.rs
4. REWRITE it to iterate env.player.active_skill_list instead of raw XML gems
5. Run: CHUNK=PERF-04-reservation DATA_DIR=./data cargo test --test chunk_oracle -- --nocapture
6. Target: 30/30 builds pass (wand_occultist should now work with Blasphemy)
7. Run: cargo test --workspace --exclude pob-wasm

Rules:
- Do NOT iterate raw XML gems. Use active_skill_list.
- Do NOT skip Lua code paths.
